...
export class AuthService {
  ...
  async register(user: userDto) {
    try {
      ...
      throw new HttpException('Email này đã có người đăng kí sử dụng rồi! Vui lòng sử dụng Email khác!', 401);
    }
  }

  async login(user: userDto) {
    const account = await this.userRepo.findOne({ where: { email: user.email } });
    if (account) {
